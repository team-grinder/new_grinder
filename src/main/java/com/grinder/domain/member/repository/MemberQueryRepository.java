...
public class MemberQueryRepository {
    ...
    public Pages<Member> getUsers(MemberSearchPage searchPage) {
        ...
        Long memberCount = query.from(memberEntity)
                .select(memberEntity.count())
                .where(booleanBuilder)
                .fetchOne();

        // 페이지네이션 처리
        List<Member> members = query.from(memberEntity)
                .select(memberEntity)
                .where(booleanBuilder)
    ...
    private void SearchForType(MemberSearchPage searchPage, BooleanBuilder booleanBuilder) {
        if (searchPage.getSearchType() == null) return; // 검색 타입이 null인 경우는 검색하지 않음

        switch (searchPage.getSearchType()) {
            case ALL:
                booleanBuilder.and(memberEntity.email.contains(searchPage.getSearchQuery())
                        .or(memberEntity.nickname.contains(searchPage.getSearchQuery()))
                        .or(memberEntity.phoneNumber.contains(searchPage.getSearchQuery())));
                break;
            case EMAIL:
                booleanBuilder.and(memberEntity.email.contains(searchPage.getSearchQuery()));
                break;
            case NICKNAME:
                booleanBuilder.and(memberEntity.nickname.contains(searchPage.getSearchQuery()));
                break;
            case PHONE:
                booleanBuilder.and(memberEntity.phoneNumber.contains(searchPage.getSearchQuery()));
                break;
            default:
    ...
    private void searchForDateRange(MemberSearchPage searchPage, BooleanBuilder booleanBuilder) {
        if (searchPage.getStartDate() != null && searchPage.getEndDate() != null) {
            ...
            booleanBuilder.and(memberEntity.createDate.between(betweenRange[0], betweenRange[1]));
        }
    }
}
