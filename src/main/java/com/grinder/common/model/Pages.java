...
public class Pages<T> {
    private final Page page; // 페이지 데이터

    private final List<T> content; // 컨텐츠 데이터

    public Pages(List<T> content, long total, long nowPage, long cntPerPage, long cntPage) {
        this.page = new Page(total, nowPage, cntPerPage, cntPage);
    ...
    // 현재 페이지, 시작 페이지, 끝 페이지, 전체 항목 수, 페이지당 항목 수, 마지막 페이지, SQL start/end
    @Getter
    public static class Page {
        ...
        public Page(long total, long nowPage, long cntPerPage, long cntPage) {
            this.total = total;
